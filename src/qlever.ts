...
export class QleverImporter implements Importer {
  ...
  async doImport(
    ...
    context.spinner.suffixText = `downloading ${distribution.accessUrl}`;
    let localFile: string;
    try {
      localFile = await this.downloader.download(distribution);
    } catch (e) {
      console.error('download failed', e);
      return new ImportFailed(distribution.accessUrl!, (e as Error).message);
    }

    context.spinner.suffixText = `indexing ${distribution.accessUrl}`;
    await this.index(
      localFile,
  ...
  async finish(): Promise<void> {
    await this.options.taskRunner.stop(this.serverTask!);
  }
}
